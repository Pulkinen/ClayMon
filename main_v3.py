...
def GimmeRigFrame(rigNum):
    ...
    frame = html.Div([
        ...
        html.H1('199.99 Mh/sec', id='hashrate '+rigN, style=dict(margin=10, width=220)),
...
layout = html.Div([
                ...
                dcc.Interval('interval-id', interval=10*1000)],
            className='col',)
app.layout = layout
...
def TakeStateJsonAndUpdateRigs():
    global rez
    if (datetime.datetime.now() - rez['lastUpdate']).total_seconds() > 5:
        # f = open('state.json', 'r')
        # strs = f.readlines()
...
def _graph_field(nint):
    ...
    if datapool == None:
        datapool = {}
        for rigName in rigs:
            datapool[rigName] = [0]*400

    data = []
    Xs = [datetime.datetime.now() - datetime.timedelta(seconds=10*x) for x in range(0, 400)]
